...
void Core_iqInsert(enum InterruptType item) {
    if (core->iqSize != INTERRUPT_QUEUE_SIZE) {
        core->interruptQueue[core->iqRear] = item;
        core->iqRear = (core->iqRear + 1) % INTERRUPT_QUEUE_SIZE;
        core->iqSize++;
    }
}

static enum InterruptType Core_iqRemove() {
    if (!core->iqSize) {
        return Interrupt_NULL;
    }
    int tmp = core->iqFront;
    core->iqFront = (core->iqFront + 1) % INTERRUPT_QUEUE_SIZE;
    core->iqSize--;
    return core->interruptQueue[tmp];
}
...
static void rla() {
    ...
    bit last = core->registers.octets.A >> 7;
    core->registers.octets.A <<= 1;
    core->registers.octets.A |= TEST_BIT(core->registers.octets.F, C_FLAG);
...
static void rra() {
    ...
    bit first = core->registers.octets.A & 1;
    core->registers.octets.A >>= 1;
    core->registers.octets.A |= (TEST_BIT(core->registers.octets.F, C_FLAG) << 7);
...
static void ld() {
    //Load (i.e. move)
    if ((FIELD_Y == 6) && (core->registers.sedectets.HL & 0xF000) == 0x3000) {
        volatile int breakHereYouIdiot = 0x69;
    }
...
static void Core_render() {
    // VRAM offset for the tile map
    uint32 screenSettings = Memory_read(core->memory, 0xFF40);
    static byte colorPicker[] = {0xFF, 0xC0, 0x60, 0x00};
    
#define SCROLL_X (screenSettings >> 16) & 0xFF
#define SCROLL_Y (screenSettings >> 24)
    
    bool mapSelect = TEST_BIT(screenSettings, 3);
    bool tileSelect = TEST_BIT(screenSettings, 4);
    short tileMap = mapSelect? 0x9C00 : 0x9800;
    short tileSet = tileSelect? 0x8000: 0x8800;
    
    for (byte line = 0; line < 144; line++) {
        
        // Background
        if ( 1 ) { //TEST_BIT(screenSettings, 0)) {
            // Which line of tiles to use in the map
            tileMap += (line + SCROLL_X) >> 3;
            
            // Which tile to start with in the map line
            byte lineOffset = (SCROLL_Y >> 3);
            
            // Where to render on the canvas
            //byte canvasOffset = line * 160 * 4;
            
            // Which line of pixels to use in the tiles
            byte y = (line + SCROLL_Y) & 7;
            ...
            byte color;
            byte tile = Memory_read(core->memory, tileMap + lineOffset) & 0xFF;
            
            // If the tile data set in use is #1, the
            // indices are signed; calculate a real tile offset
            //tile += (mapSelect && tile < 128)? 256: 0;
            
            for (byte pixel = 0; pixel < 160; pixel++) {
                
                short trueTile = Memory_read(core->memory, tileSet + (tile << 4) + (line << 1)) & 0xFFFF;
                
                // Re-map the tile pixel through the palette
                color = colorPicker[((trueTile >> (pixel % 8)) & 1) | (((trueTile >> (pixel % 8 + 8)) & 1) << 1)];
                
                // Plot the pixel to canvas
                // core->framebuffer[pixel][line] = color;
                //canvasOffset += 4;
                
                x += 1;
                
                // When this tile ends, read another
                if (x == 8) { // Change tile /*TODO*/
                    x = 0;
                    lineOffset = (lineOffset + 1) & 0x1F;
                    tile = Memory_read(core->memory, tileMap + lineOffset) & 0xFF;
                }
            }
        }
        
        // Sprites
        if ( 1 ) { //TEST_BIT(screenSettings, 1)) {
            for (byte i = 0; i < 80; i += 4) {
                uint32 sprite = Memory_read(core->memory, 0xFE00 + i);
                
                byte y = ((byte*) &sprite)[0]; // I legitimately feel guilty
                byte x = ((byte*) &sprite)[1];
                byte tile = ((byte*) &sprite)[2];
                
                PICBOY_LOG("line %i, sprite %i\n", line, sprite);
                
                // Check if this sprite falls on this scanline
                if (y <= line && y + 8 > line) {
                    // TO-DO: Palette to use for this sprite
                    // var pal = obj.pal ? GPU._pal.obj1 : GPU._pal.obj0;
                    
                    // Where to render on the canvas
                    //byte canvasoffs = (line * 160 + x) * 4;
                    
                    // Data for this line of the sprite
                    short trueTile;
                    
                    // TO-DO: If the sprite is Y-flipped,
...
//                    {
                    ...
                    trueTile = Memory_read(core->memory, tileSet + (tile << 4) + (line << 1)) & 0xFFFF;
                    //}
                    ...
                    for (byte pixel = 0; pixel < 8; pixel++) {
                        ...
                        if (
                            (x + pixel) >= 0 && (x + pixel) < 160 &&
                            // TEST_BIT(trueTile, pixel) &&
                            (1 /* TODO: PRIORITY */ /* || TODO: Check background !scanrow[obj.x + x]*/)
                        ) {
                            // TODO :If the sprite is X-flipped,
                            // write pixels in reverse order
                            // colour = pal[tilerow[obj.xflip ? (7-x) : x]];
                            
                            bit colorBit0 = (trueTile >> pixel) & 1;
                            bit colorBit1 = (trueTile >> (pixel + 8)) & 1;
                            
                            core->framebuffer[x][line] = colorPicker[colorBit0 | (colorBit1 << 1)];
//                            core->framebuffer[x][line + 1] = colour[1];
//                            core->framebuffer[x][line + 2] = colour[2];
//                            core->framebuffer[x][line + 3] = colour[3];
//
                            //canvasoffs += 4;
                        }
                    }
...
static word ivt[] = { 0x0000, 0x0040, 0x0048, 0x0050, 0x0058, 0x0060 };

void Core_cycle() {
    enum InterruptType removed;
    while ((removed = Core_iqRemove()) && core->interrupts && TEST_BIT(core->memory->interruptRegister, (removed - 1))) {
        
        if (removed == Interrupt_vBlank) {
            Core_render();
        }
            
        core->registers.sedectets.SP -= 2;
        Memory_write(core->memory, core->registers.sedectets.SP, core->registers.sedectets.PC, true);
        core->registers.sedectets.PC = ivt[removed];
    }
